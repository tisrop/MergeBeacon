...
![MergeBeacon 跨平台 PR 收件箱](docs/images/review-inbox.png)

### PR / MR 列表

![MergeBeacon PR 列表与合并状态](docs/images/pull-request-list.png)
...
5. 在文本 Diff 中使用 `⌘/Ctrl + F` 查找代码；图片文件可切换到 **预览** 查看变更前后内容。
6. 选中代码并打开快速评论框；三个平台均支持提交行级评论。
7. 在 **评审意见** 页签查看线程，按解决状态筛选、回复或跳转未解决线程；仅在平台支持时
   解决或重新打开线程。
8. 在文件树标记已查看状态，并在提交前确认未查看文件和未解决线程提示。
9. 需要合并时选择当前平台支持的策略，并确认关联 Issue 的关闭选项。
...
3. 在变更预览中检查 Commit 和 Diff；需要时按单个 Commit 切换 Diff 范围。预览会按页继续加载，
   但超过客户端安全上限或平台无法继续返回数据时会标记为不完整。
...
- 创建页的 Commit / Diff 预览受平台 Compare API 和客户端分页安全上限限制；检测到截断、
  后续分页失败或达到上限时会显示不完整警告，用户确认范围后仍可创建 PR / MR。
- 已关闭或已合并的历史列表受平台分页能力限制：GitHub 最多浏览前 1,000 条，GitLab 和 Gitee
  无法继续分页时会明确提示。
...
本项目关注开源协作与技术交流。欢迎访问 [LINUX DO](https://linux.do/) 社区，与更多开发者交流、
分享经验，并发现有趣的开源项目。
