...
5. **默认安全**：密钥、Token、远端文本和自托管地址按不可信输入处理；禁止为了便利降低 CSP、
   明文存储凭据或绕过白名单清洗直接使用 `v-html`。
6. **不静默降级能力**：平台不支持的动作必须由前端和后端共同拒绝，并向用户展示清晰原因。
7. **双语产品体验**：用户界面文案必须通过 `src/i18n/` 的简体中文和英文资源渲染，默认使用
   简体中文；AI 评审 Prompt 必须按请求携带的语言生成。代码标识符、协议字段和官方产品名沿用英文。
...
- PR、Issue、仓库和认证请求发起时必须捕获平台、仓库、筛选、排序、分页游标与请求序列号；响应
  写入前重新核对当前上下文。旧请求或其他平台的迟到响应不得覆盖新状态。
...
- AI 流式事件只处理当前 `request_id`。开始新评审和组件卸载时必须取消旧任务并解除监听；用户
  主动取消属于正常生命周期，不显示为错误。
- PR 当前页状态补充必须为每个批次生成 `request_id`；筛选、排序、分页、仓库上下文变化或组件
  卸载时必须取消旧批次，并只接收当前请求的结果。
- Vue 组件注册的事件监听器、定时器和异步订阅必须在卸载时清理；Rust 可取消任务必须使用 generation
  防止旧任务移除新句柄，并分别从 `AppState.ai_tasks` 或 `AppState.pr_list_status_tasks` 正确注册、取消
  和移除。
...
- PR / MR 列表的标题、作者、标签、评审状态、负责人/测试者筛选与排序必须统一通过 `PrListQuery`
  传递，并由各平台 adapter 映射为服务端查询；不得在前端用已加载页模拟全量远端筛选。
...
- 模型 ID、标题、错误正文及不声明为 Markdown 的远端字符串必须按纯文本渲染。Markdown 内容只能
  通过共享 `MarkdownRenderer.vue` 解析，并在标签、属性和 URL 协议白名单清洗后使用受控
  `v-html`；业务组件禁止直接注入远端 HTML。
- 远程图片、视频和 Markdown 媒体附件必须按功能需要限制协议、host、path、扩展名和大小；GitHub
  视频附件加载失败时必须保留原始安全链接，禁止因预览失败丢失可访问目标。
- AI Prompt 截断必须位于 UTF-8 字符边界，并保持约 64 KiB 上限；禁止直接按字节索引字符串。
- CSP 只增加功能所需的最小来源；修改 `img-src`、`media-src` 或其他资源来源时必须同步核对运行时
  allowlist，禁止允许远程脚本或使用宽泛通配符规避资源限制。
...
- 以下改动必须同步增加回归测试：
  - 认证恢复、平台切换和登录目标；
  - 仓库分页、去重、重试和迟到响应；
  - PR / MR 服务端搜索、组合筛选、排序、分页与状态批次取消；
  - PR/Issue 异步上下文；
  - Diff 图片/视频预览边界、object URL 清理与 Markdown 附件来源；
