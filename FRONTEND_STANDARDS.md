...
- 可预览的图片和视频必须提供源码/媒体视图切换，并明确处理 loading、过大、无效、不可用和加载失败
  状态；普通文件内容与媒体内容必须遵守后端返回的截断边界，不得在前端拼接误导性结果。
- 媒体预览的 object URL、事件监听和旧请求必须在切换文件或卸载时清理；旧响应不得覆盖当前文件。
  SVG 不得作为标记注入页面。
- 扩展 Markdown 标签、属性、链接协议或交互前，必须同步评审 HTML 清洗、外链策略、CSP、键盘
  操作和回归测试；脚本、样式、事件属性和危险 URL 协议不得进入渲染结果。Markdown 视频附件只允许
  受限的 GitHub HTTPS 来源，加载失败时必须保留原始安全链接。
...
- 异步响应写入状态前必须验证平台、仓库、筛选、排序、分页上下文、序列号或 `request_id`；视觉
  优化不得移除这些竞态保护。可取消的 PR 状态补充请求必须在新筛选、翻页、切换仓库和卸载时取消
  旧批次，并忽略非当前 `request_id` 的结果。
...
- 用户可见界面文案必须通过 `src/i18n/` 提供简体中文和英文资源，禁止在组件中新增只支持单一
  语言的固定文案。默认语言为简体中文；平台、协议、代码标识和官方产品名保留其标准英文写法。
- 切换界面语言必须立即更新可见文案和 `<html lang>`，并持久化到本地；布局必须同时验证中英文长度。
- 原生菜单文案同样必须来自 `src/i18n/`：首次安装使用简体中文 fallback，语言切换后立即同步完整菜单；
  菜单动作的 JS bridge、路由和设置页 hash 必须与界面入口保持一致。
...
- 条件显示的危险操作或平台能力；
- 会影响平台、仓库、筛选、排序、分页、请求序列号或 AI `request_id` 的交互；
- PR 当前页状态补充的批次取消、迟到响应和错误回退；
- Diff 图片/视频预览、媒体大小边界、object URL 清理和 Markdown 附件来源；
- 原生菜单动作、双语文案同步和路由入口；
- 修复可访问性、监听器泄漏或异步竞态缺陷。
